...
void TransportModule::init(GLuint shaderProgram, GLuint panelShader) {
    ...
    QString base = QString("/home/snorky/EvoPlayer/assets/buttons/");
...
void TransportModule::handleMousePress(float nx, float ny) {
    for (int i = 0; i < 6; i++) {
        ...
        {
            b.pressAnim = 1.0f;
            QProcess::startDetached("paplay", {"/home/snorky/EvoPlayer/assets/click.wav"});
            switch (i) {
...
void TransportModule::drawOverlay(int btnIdx, GLuint tex) {
    ...
    float x0 = ((btn.x - btn.w) / 2029.0f) * 2.0f - 1.0f;
    float x1 = ((btn.x + btn.w) / 2029.0f) * 2.0f - 1.0f;
    float y0 = 1.0f - ((btn.y + btn.h) / 508.0f) * 2.0f;
    float y1 = 1.0f - ((btn.y - btn.h) / 508.0f) * 2.0f;
    float verts[] = {
        x0, y0, 0.0f, 0.0f,
        x1, y0, 1.0f, 0.0f,
        x1, y1, 1.0f, 1.0f,
