...
bool DisplayModule::init(GLuint textShaderProgram, GLuint panelShaderProgram) {
    ...
    loadFontInto((qgetenv("EVOPLAYER_BASE").isEmpty() ? QString::fromLocal8Bit(qgetenv("HOME")) + "/EvoPlayer/" : QString::fromLocal8Bit(qgetenv("EVOPLAYER_BASE")) + "/") + "assets/fonts/DSEG7Classic-Bold.ttf", 32, m_faceDigital, m_charsDigital);
...
GLuint DisplayModule::loadDisplayShader() {
    // Vertex
    QFile vf((qgetenv("EVOPLAYER_BASE").isEmpty() ? QString::fromLocal8Bit(qgetenv("HOME")) + "/EvoPlayer/" : QString::fromLocal8Bit(qgetenv("EVOPLAYER_BASE")) + "/") + "shaders/display_vertex.glsl");
    ...
    // Fragment
    QFile ff((qgetenv("EVOPLAYER_BASE").isEmpty() ? QString::fromLocal8Bit(qgetenv("HOME")) + "/EvoPlayer/" : QString::fromLocal8Bit(qgetenv("EVOPLAYER_BASE")) + "/") + "shaders/display_fragment.glsl");
